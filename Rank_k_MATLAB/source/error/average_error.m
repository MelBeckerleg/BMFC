...
function [MAE, RMSE,AUC,AUCpr,curvex,curvey] = average_error(y_pred, y_true, lims_init, lims_scaled, round_pred)
...
if round_pred
    ...
    y_pred = round(y_pred);
    %y_pred=(y_pred>sum(lims_scaled)/length(lims_scaled))*1;
end
...
[curvex,curvey,~,AUC]=perfcurve(y_true,y_pred,1);
[~,~,~,AUCpr]=perfcurve(y_true,y_pred,1,'xcrit','reca','ycrit','prec');
