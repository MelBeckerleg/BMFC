...
homepath='/home/beckerleg/' ;%
impath='/home/user/Documents/Mel/' ;%
homepath='/home/user/Documents/Mel/' ;%
...
for prot_fts={'cathids','none'}%,'funfams','sequences','none'}
    for cmp_fts={'fingerprints','none'}%'fingerprints'
        ...
        m=30;n=30;
        if set_up 
            load([homepath 'Ethera/MP2/Data/read_ch/mats/ch_dense_set'],'supported_ch','ch_fingerprints','ch_protein_funfams','ch_protein_sigs'); 
            load([homepath 'Ethera/MP2/Data/read_ch/mats/ch_protein_cathids.txt'])
     ...
     %   for weight_val= linspace(0,1,25)
      %      for rho_val=linspace(0,1,25)
       ...
       %     error(ii,jj)=temp(end)
        ...
        fn=[homepath sprintf( 'Ethera/stat_MC_regularised_cmp%s_prot%s_small_gammn%s_gamr%s_gamc%s_rho%s_m%s_n%s.mat',string(cmp_fts),string(prot_fts),...
