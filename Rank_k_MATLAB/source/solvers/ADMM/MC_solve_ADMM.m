...
%except for GRALS
Omega=reshape(prob_params.mask_train,prob_params.size_X);
...
C_oppt2 = @(y) AtA_op(y) + vec(rho_ADMM * reshape(y, size_X));
...
for i = 1 : maxit
  ...
  %  fprintf('iter %d:  ', i);
    ...
    % TODO: find a better way to solve this system !!!
        %%This is unpreconditioned CG
            %[y, ~, ~, ~, resvec] = pcg(C_op, vec(AtAM + rho_ADMM * (X + Z)), pcg_tol, 30, [], [], vec(X));
        %%Preconditioned CG
            [y, ~, ~, ~, resvec] = pcg(C_op, vec(AtAM + rho_ADMM * (X + Z)), pcg_tol, 30, D_inv_op, [], vec(X));
            %[y] = pcg(C_op, vec(AtAM + rho_ADMM * (X + Z)), [], 100 );
        %y=1*(y>sum(y_lims_scaled)/2);
        %%alternatively, GRALS CG (fast hessian multiplication)
    %[Y, ~, ~]=grals(gamma_c*Lc, gamma_r*Lr, Omega, AtAM, max(info_nuclear.rank,1));y=Y(:);y=1*(y>sum(y_lims_scaled)/2);
    Y = reshape(y, size_X);
    %% STEP 3: Z <- Z + X - Y
    Z = Z + X - Y;
    ...
    if not(isempty(m_val))
        ...
        [stat.mae_val(i), stat.rmse_val(i),stat.auc_val(i),stat.aucpr_val(i),stat.curvex_val,stat.curvey_val] = average_error(X(mask_val), m_val, y_lims_init, y_lims_scaled, 0);
    ...
    if not(isempty(m_test))
        % MAE and RMSE after linear mapping back to [1, 5]
        [stat.mae_test(i), stat.rmse_test(i),stat.auc_test(i),stat.aucpr_test(i),stat.curvex_test,stat.curvey_test] = average_error(X(mask_test), m_test, y_lims_init, y_lims_scaled, 0);
        % MAE and RMSE after linear mapping to [1,5] AND rounding on {1, ...,5}
