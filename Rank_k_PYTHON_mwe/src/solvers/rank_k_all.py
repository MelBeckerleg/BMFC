...
def init_noisyblock(M,gopts,*empty):
    m,n=np.shape(M);ratio=gopts['ratio'];
    randseed=gopts['random_seed']
    np.random.seed(randseed)
    x = np.zeros([1, m])[0]
    y = np.zeros([1, n])[0]
    y[np.random.permutation(n)[0:int(np.floor(n*ratio))]]=1
    x[np.random.permutation(m)[0:int(np.floor(m*ratio))]]=1
    return x,y
...
def init_y_greedy2(a, *empty):
    (n, m) = np.shape(a)
    # greedy graph type approach not the same as don't enforce a balancd partition
    select = int(np.floor(np.random.rand(1) * n))
    row = a[select, :]
    # identify overlap and sort by which ones have most positive overlap [remove the >0s if you want most overlap/closest hamming]
    rowsumsorted = np.transpose(np.argsort(sum(np.transpose(np.transpose(row) * (a)))))
    x = np.ravel(np.zeros([n, 1]))
    x[rowsumsorted[0:int(np.floor(n / 2))]] = 1
    rows = a[rowsumsorted[int(np.floor(n / 2)):], :]
    # print(rows)
    y = (np.mean(rows, 0) > 0.0) * 1.
    return x, y


def init_y_svd(a,*empty):
